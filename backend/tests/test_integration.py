...
class TestEndToEndChatFlow:
    ...
    def test_complete_chat_and_feedback_flow(
        ...
        # 3. 验证反馈已保存到数据库
        feedback = test_db.query(Telemetry)\
    ...
    def test_concurrent_users_chat(
        ...
        for user_id in users:
            chat_request = {
                "character": "tocqueville",
                "messages": [
                    {"role": "user", "content": f"来自 {user_id} 的问题"}
                ],
                "stream": False
            }
...
class TestDataConsistency:
    ...
    def test_feedback_data_integrity(
        ...
        # 验证数据正确保存
        feedback = test_db.query(Telemetry)\
            .filter(Telemetry.user_id == sample_user_id)\
            .first()
    ...
    def test_statistics_accuracy(
        ...
        # 验证平均值
        total_score = sum(rating * count for rating, count in rating_distribution.items())
