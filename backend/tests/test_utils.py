...
class TestCustomExceptions:
    ...
    def test_app_exception_basic(self):
        """测试基础应用异常"""
        exc = AppException(message="测试错误", code="TEST_ERROR")
    ...
    def test_app_exception_with_details(self):
        """测试带详情的应用异常"""
        exc = AppException(
            message="测试错误", code="TEST_ERROR", details="详细错误信息"
        )
    ...
    def test_app_exception_custom_status_code(self):
        """测试自定义状态码"""
        exc = AppException(message="测试错误", code="TEST_ERROR", status_code=400)

        assert exc.status_code == 400

    def test_validation_exception(self):
        """测试验证异常"""
        exc = ValidationException(message="验证失败", details="字段 X 不能为空")
    ...
    def test_ai_service_exception(self):
        """测试 AI 服务异常"""
        exc = AIServiceException(message="AI 服务错误", details="模型不可用")
    ...
    def test_database_exception(self):
        """测试数据库异常"""
        exc = DatabaseException(message="数据库连接失败", details="无法连接到数据库")
...
class TestSchemaValidation:
    ...
    def test_chat_request_empty_messages(self):
        ...
        with pytest.raises(Exception):
            ChatRequest(character="tocqueville", messages=[], stream=False)
...
class TestDatabaseConnection:
    ...
    def test_database_transaction_commit(self, test_db):
        ...
        # 验证记录已保存
        result = (
            test_db.query(Telemetry).filter(Telemetry.user_id == "test-commit").first()
        )
...
class TestErrorHandling:
    ...
    def test_422_validation_error(self, client: TestClient):
        """测试 422 验证错误"""
        # 发送无效数据
        response = client.post("/api/v1/telemetry", json={"invalid": "data"})
...
class TestUtilityFunctions:
    ...
    def test_assert_error_response(self):
        ...
        # 有效错误响应
        error_data = {"error": {"code": "TEST_ERROR", "message": "测试错误"}}
