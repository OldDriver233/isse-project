...
class TestChatEndpoint:
    ...
    @patch("app.services.ai_service.ai_service.chat")
    def test_chat_invalid_character(self, mock_chat: AsyncMock, client: TestClient):
        """测试无效的角色名称"""
    ...
    def test_chat_empty_messages(self, client: TestClient):
        """测试空消息列表"""
        request_data = {"character": "tocqueville", "messages": [], "stream": False}
    ...
    def test_chat_ai_service_unavailable(
        self, mock_chat: AsyncMock, client: TestClient, sample_chat_request: dict
    ):
        """测试 AI 服务不可用"""
...
class TestChatStreamingEndpoint:
    ...
    @patch("app.services.ai_service.ai_service.chat")
    def test_chat_stream_request(self, mock_chat: AsyncMock, client: TestClient):
        """测试流式请求"""
