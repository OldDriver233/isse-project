...
class TestTelemetryModel:
    ...
    def test_telemetry_missing_required_fields(self, test_db: Session):
        ...
        with pytest.raises(Exception):
            telemetry = Telemetry(
                overall_rating=8, messages='[{"role": "user", "content": "测试"}]'
            )
            test_db.add(telemetry)
            test_db.commit()
        ...
        with pytest.raises(Exception):
            telemetry = Telemetry(
                user_id="test-user", messages='[{"role": "user", "content": "测试"}]'
            )
            test_db.add(telemetry)
            test_db.commit()
    ...
    def test_query_by_rating(self, test_db: Session):
        ...
        # 查询低评分
        low_ratings = (
            test_db.query(Telemetry).filter(Telemetry.overall_rating <= 5).all()
        )
...
class TestChatSessionModel:
    ...
    def test_chat_session_default_message_count(self, test_db: Session):
        """测试默认消息数量"""
        session = ChatSession(id="session-default", character="tocqueville")
    ...
    def test_chat_session_repr(self, test_db: Session):
        """测试字符串表示"""
        session = ChatSession(
            id="session-repr", character="tocqueville", message_count=10
        )
    ...
    def test_query_by_character(self, test_db: Session):
        ...
        for i, character in enumerate(["tocqueville", "common", "tocqueville"]):
            session = ChatSession(
                id=f"session-{i}", character=character, message_count=i
            )
            test_db.add(session)
    ...
    def test_update_message_count(self, test_db: Session):
        """测试更新消息数量"""
        session = ChatSession(
            id="session-update", character="tocqueville", message_count=0
        )
