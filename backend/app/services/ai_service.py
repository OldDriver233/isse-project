...
class AIService:
    ...
    def _get_namespace(self, character: str) -> str:
        ...
        print(
            f"⚠️ 角色 '{character}' 不在可用列表中，使用默认 namespace: {settings.DEFAULT_NAMESPACE}"
        )
...
"""
    ...
    def _build_rag_prompt(
        self, question: str, context: str, namespace: str
    ) -> ChatPromptTemplate:
        """
        构建 RAG 提示词
...
"""
    ...
    async def _generate_non_stream(
        self, prompt: ChatPromptTemplate, character: str
    ) -> ChatResponse:
        """
        生成非流式响应
    ...
    async def _generate_stream(
        ...
        try:
            ...
            first_chunk = StreamResponse(
                result=StreamResult(
                    delta=StreamDelta(role="assistant", content=""), finish_reason=None
                ),
                usage=None,
            ...
            for chunk in chain.stream({}):
                if hasattr(chunk, "content") and chunk.content:
                    stream_chunk = StreamResponse(
                        result=StreamResult(
                            delta=StreamDelta(content=chunk.content), finish_reason=None
                        ),
                        usage=None,
                        created=created_timestamp,
                        id=response_id,
            ...
            final_chunk = StreamResponse(
                result=StreamResult(
                    delta=StreamDelta(content=""), finish_reason="stop"
                ),
                usage=None,
                created=created_timestamp,
                id=response_id,
            ...
            print(f"❌ 流式响应生成失败: {e}")
            # 发送错误信息
            error_data = {"error": {"code": "STREAM_ERROR", "message": str(e)}}
